...
public class BOJ2164 {
	...
	public static void main(String[] args) {
		...
		Queue< Integer> q = new LinkedList<Integer>(); //큐를 만든다
		 ...
		 for (int i = 1; i <= n; i++) {
				q.add(i); //큐에 카드번호를 차례로 넣는다
				//배열을 이용하지 않고도 바로 큐에 넣을 수 있고 불필요하게 배열을 사용하지 않기 때문에 메모리도 사용량도 줄일 수 있다
				//큐와 배열은  둘 다 데이터 저장을 위해 사용되지만 배열은 일반적인 데이터 저장 및 빠른 접근을 위해,
				//큐는 데이터를 맨 앞에서 제거하고 맨 뒤에서 추가하는 동작만 허용하고 인덱스로 임의 접근이 불가능하기 때문에
				//큐는 순서가 중요한 작업을 처리하기 위해 사용된다
